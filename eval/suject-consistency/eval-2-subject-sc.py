...
class GroundinoScorer(torch.nn.Module):
    ...
    def __call__(self, prompt, images, ref_images_1, ref_images_2, category1, category2):
        if category1[0]==category2[0]:
            images_1 = [segment_pil_samecategory(self.grounding_model,image,cate1,0) for image,cate1 in zip(images,category1)]
            images_2 = [segment_pil_samecategory(self.grounding_model,image,cate2,1) for image,cate2 in zip(images,category2)]
        else:
